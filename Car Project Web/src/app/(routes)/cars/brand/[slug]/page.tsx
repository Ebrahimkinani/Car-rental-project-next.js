...
// Filter cars based on search criteria and brand
function filterCars(cars: Car[], brandId: string, filters: { carName: string; carModel: string }): Car[] {
  return cars.filter((car) => {
    // Filter by brand first
    const brandMatch = car.brandId === brandId;
...
export default function BrandPage({ params }: BrandPageProps) {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const carsData = await getCarsByBrandFromStorage(categoryData.id);
        if (mounted) {
          setCars(carsData);
        }
      } catch (error) {
  ...
  // Filter cars based on brand and search criteria
  const filteredCars = useMemo(() => {
    if (!category) return [];
    return filterCars(cars, category.id, searchFilters);
  }, [category, cars, searchFilters]);
    ...
    <main className="min-h-screen bg-white">
      <div className="pt-32 pb-16">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <BrandSearch onSearch={handleSearch} brandId={category.id} cars={cars} />
          ...
          {filteredCars.length > 0 ? (
            ...
            <div className="text-center py-12">
              <div className="text-gray-500 text-lg mb-4">
                No {category.name} cars available at the moment
              </div>
