...
export default function CarGrid({ cars, className = "" }: CarGridProps) {
  ...
  // Responsive car limits: 4 cars (2×2) on small, 6 cars (2×3) on medium, 8 cars (2×4) on large
  const getDisplayLimit = () => {
    if (isDesktop) return 8; // 2 rows × 4 columns
    if (isTablet) return 6; // 2 rows × 3 columns
    return 4; // 2 rows × 2 columns
  };
